...
def _tmplt_access_list_type(config_data):
    command = "access-list {acls_name} {acl_type}".format(**config_data)
    q(command)
    return command
    ...
    PARSERS = [
        ...
        },
        {
            ...
            "getval": re.compile(
                r"""^access-list*
                    \s*\S+\s*line*
                    \s*\S+\s*(?P<acl_ext>extended)|(?P<acl_std>standard)
                    """,
                re.VERBOSE,
            ),
            "setval": _tmplt_access_list_type,
            "result": {
                "acls": {
                            "{{ acl_name }}": {
                            "acl_type": "{{ acl_ext if acl_ext is defined else acl_std if acl_std is defined }}",
                        }
                }
            },
        },
        {
            ...
            "getval": re.compile(
                r"""^access-list*
                    ...
                    \s*extended*\s*(?P<grant>\S+)
                    """,
                re.VERBOSE,
            ),
            #"setval": _tmplt_access_list_entry_line,
            "result": {
                "acls": {
                   "{{ acl_name }}": {
                        "aces": [
        ...
        #         re.VERBOSE,
        #     ),
        #     #"setval": _tmplt_access_list_entry_grant,
        #     "result": {
        #         "acls": {
        #             "aces": [{
        #                  "{{ acl_name }}": {
